...
| サービスファイル名    | 説明                                                                     |
| ------------------- | ------------------------------------------------------------------------ |
| ItemService.php     | 商品一覧の検索・フィルタリング・セッション管理などの複雑なクエリロジックを担当  |
| PurchaseService.php | 商品購入におけるDB保存処理、およびトランザクション管理を担当                  |
| StripeService.php   | Stripe APIを使用した決済セッションの作成、および外部決済連携の窓口を担当       |
...
本アプリでは、商品購入時の決済にStripe のテストモードを利用しています。
以下のテスト用カード情報を使用して、実際に決済フローを確認いただけます。
...
| カード名義人	           | 任意の英字（例: TARO TANAKA）|
| 郵便番号	              | 任意の数字（例: 123-4567）  |

※注意: 実際のクレジットカード情報は絶対に入力しないでください。
